...
// --- 2. Функционал: Отображение товаров (БЕЗ ФОТО) ---
...
// Логика кнопки "Купить" (ФИНАЛЬНАЯ ВЕРСИЯ: С уведомлением и ссылкой на продавца)
function buyProduct(id, name, price) {
    ...
    // Проверка на ошибку WebAppMethodUnsupported и открытие чата
    if (tg && tg.openTelegramLink) {
        // 1. Открываем чат
        tg.openTelegramLink(telegramUrl);
        // 2. Выводим уведомление для подтверждения (появится для ВСЕХ товаров)
        tg.showAlert(`Запрос на покупку ${name} отправлен продавцу @${sellerUsername}. Откроется чат.`);
    } else {
        // Если запущено не в Telegram
